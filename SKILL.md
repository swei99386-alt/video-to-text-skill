---
name: 视频转文字
description: 把视频/播客/演讲里说的话提取成文字稿；显式加 --visual 时，额外生成关键帧、OCR 和时间线证据包。给一条链接(X/Twitter、YouTube、B站等)或本地视频音频文件即可。触发词:提取文字、扒字幕、转文字稿、把这个视频的话弄成文字、这段视频讲了啥、transcribe、字幕下载
---
...
下面命令里的 `<SKILL_DIR>` 指当前这份 `SKILL.md` 所在的 Skill 文件夹。不同工具的安装目录可能不同，运行前替换成实际路径，不要调用其他智能体的同名副本。

```
python "<SKILL_DIR>/scripts/grab.py" "<链接或本地文件>" "输出文件夹名" [语言]
```
...
## 可选视觉证据模式（只在用户明确要求时开启）

默认流程不变。用户要求补回画面信息时，在原命令末尾加 `--visual`：

```
python "<SKILL_DIR>/scripts/grab.py" "<视频链接或本地视频>" "输出文件夹名" [语言] --visual
```

视觉旁路优先调用固定版本的 `mcp-video-analyzer@0.8.0`，不替换现有字幕优先和 faster-whisper 转写链。它会把场景关键帧、定时补采帧、OCR 和时间线整理到同一个输出目录。

新增产物：

- `视频理解报告.md`：区分原始画面证据、OCR、时间线和模型解释。
- `视觉证据.json`：带 `evidence_id`、毫秒时间、关键帧路径和生成器信息的结构化证据。
- `运行清单.json`：记录来源、命令、耗时、数量和警告。
- `关键帧/`：可回看的 JPEG 画面。

注意：

- 不加 `--visual` 时，旧命令和旧产物不应触发 Node 或视觉分析。
- `--visual` 只接受视频链接或视频文件；纯音频输入会明确跳过视觉阶段。
- 当前视觉旁路覆盖可访问的视频链接和本地视频文件；只能在网页/App 里播放、无法取得视频文件的来源，现阶段仍只走 `record.py` 的纯音频录制，屏幕视觉采集需要单独的后续任务。
- 视觉阶段默认使用本地 CLI；如果 `npx`、Node.js 18+ 或候选项目不可用，应报告失败，不得静默生成想象的画面说明。
